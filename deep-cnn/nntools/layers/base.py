...
class MaxPool1DLayer(Layer):
    def __init__(self, input_layer, ds, ignore_border=False):
        super(MaxPool1DLayer, self).__init__(input_layer)
        self.ds = ds # a tuple
    ...
    def get_output_shape_for(self, input_shape):
        ...
        if self.ignore_border:
            output_shape[1] = int(np.floor(float(output_shape[1]) / self.ds[0]))
            output_shape[2] = int(np.floor(float(output_shape[2]) / self.ds[1]))
        else:
            output_shape[1] = int(np.ceil(float(output_shape[1]) / self.ds[0]))
            output_shape[2] = int(np.ceil(float(output_shape[2]) / self.ds[1]))
